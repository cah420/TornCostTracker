...
Migration 011 adds Trade Resolution between canonical trade evidence and canonical paid acquisitions. `TradeResolutionService` correlates only verified terminal completion, cash, and item directions; `TradeResolutionRepository` owns immutable revisions and transactional supersession; and `TradeResolutionProjector` converts only an active balanced revision into acquisitions. The UI can request or edit a resolution but cannot write downstream accounting. Activating a revision invalidates the current Projection/Ledger/Cost Lot/FIFO/Inventory Position chain in the same transaction, preventing superseded allocations from remaining active.
...
Migration 012 adds immutable Disposal Resolution revisions, durable resolution-to-canonical-event links, and derived Realized Results. Verified direct disposals and active correlated resolutions project through the existing Projection and Ledger layers. FIFO remains the sole Cost Lot consumption boundary, and Inventory Position remains a read-only derivation of lots and FIFO consumption.
