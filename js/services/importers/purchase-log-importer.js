...
const TITLE = {
  ...
  factionGiftReceive: /^faction give item receive$/i,
  cityItemFind: /^city item find$/i,
...
function logRecords(logs){
  if (Array.isArray(logs)) return logs;
  // Some Torn responses key records by the stable log ID instead of repeating
  // it in the value. Preserve that ID for acquisition deduplication.
  if (logs && typeof logs === "object") {
    return Object.entries(logs).map(([id, log]) => ({ ...log, id: log?.id ?? log?.log_id ?? id }));
  }
  return [];
}
...
function logCollection(response){
  ...
  return logRecords(response?.logs ?? response?.log);
}
...
function counterpartyFor(data){
  return number(data?.seller_id ?? data?.sender ?? data?.user_id ?? data?.player_id ?? data?.partner_id ?? data?.counterparty_id);
}
...
function normalizeFreeAcquisition(log, sourceType){
  const data = dataFor(log);
  const id = logId(log);
  const timestamp = timestampFor(log);
  if (id === null || timestamp === null) return null;
...
function isHandledTitle(title){
  return TITLE.bazaarPurchase.test(title) ||
    ...
    TITLE.factionGiftReceive.test(title) ||
    TITLE.cityItemFind.test(title) ||
...
function normalizedRecords(logs){
  ...
  logs.forEach((log) => {
    ...
    if (TITLE.factionGiftReceive.test(title)) records.push(normalizeFreeAcquisition(log, "factionGift"));
    if (TITLE.cityItemFind.test(title)) records.push(normalizeFreeAcquisition(log, "cityFind"));
  });
