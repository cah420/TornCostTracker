...
class FifoMemory {
  constructor(){ this.demands = new Map(); this.consumptions = new Map(); this.realized = new Map(); this.dispositions = new Map(); this.diagnostics = new Map(); this.run = null; }
  ...
  async prepareConsumptionReplacement(fifoVersion, itemId){ const ids = new Set([...this.consumptions.values()].filter((row) => row.fifoVersion === fifoVersion && row.itemId === String(itemId)).map((row) => row.id)); [...this.consumptions.entries()].forEach(([id, row]) => { if (row.fifoVersion === fifoVersion && row.itemId === String(itemId)) this.consumptions.delete(id); }); [...this.realized.entries()].forEach(([id, row]) => { if (row.fifoVersion === fifoVersion && row.itemId === String(itemId)) this.realized.delete(id); }); return ids; }
  ...
  async storeRealizedResults(rows){ rows.forEach((row) => this.realized.set(row.id, row)); }
  ...
  async countRows(){ return { demands: this.demands.size, consumptions: this.consumptions.size, dispositions: this.dispositions.size, diagnostics: this.diagnostics.size, realizedResults: this.realized.size }; }
