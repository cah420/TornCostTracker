...
export class InventoryPositionRepository {
  ...
  async finishRun(id, { status, metrics, sourceProjectionVersion = null, errorSummary = null, completedAt = Date.now() }){ const statements = [{ sql: "UPDATE accounting_inventory_position_runs SET status = ?, completed_at = ?, source_projection_version = COALESCE(?, source_projection_version), metrics_json = ?, error_summary = ? WHERE id = ?", bind: [status, completedAt, sourceProjectionVersion, inventoryPositionPayload(metrics), errorSummary, id] }]; if (status === "completed") statements.push({ sql: "DELETE FROM accounting_rebuild_state WHERE layer = 'inventory_position'" }); await this.database.transaction(statements); }
